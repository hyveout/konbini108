...
function SplashScreen({ onEnter }) {
  return (
    <div className="screen splash-bg scanlines noise" style={{
      ...
      paddingTop: 24, paddingBottom: 'max(40px, env(safe-area-inset-bottom))',
      position: 'relative',
    }}>
      <PhoneChrome />

      {/* Lanterns row (alley vibe) */}
      <div className="splash-lanterns" style={{
        ...
        {[0,1,2,3,4].map(i => (
          <div key={i} style={{
            ...
            borderRadius: '50%',
            boxShadow: '0 0 20px rgba(255,200,100,0.6), 0 -10px 0 -8px #2a2a2a',
            opacity: 0.85,
            animation: `wobble ${2 + i * 0.3}s ease-in-out infinite`,
          }}/>
        ))}
      </div>

      {/* Main sign */}
      <div style={{ marginTop: 70, textAlign: 'center', position: 'relative', zIndex: 10 }}>
        <div style={{
          display: 'inline-flex', alignItems: 'center', gap: 8,
          background: '#fff', color: 'var(--red)',
          padding: '4px 14px',
          fontFamily: 'Bebas Neue', fontSize: 16, letterSpacing: '0.2em',
          marginBottom: 14,
        ...
        <div className="splash-title" style={{
          ...
          textShadow: '0 0 30px rgba(255,214,10,0.5), 4px 4px 0 var(--red)',
          fontWeight: 900,
        }}>
          KONBINI
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: 8 }}>
            <Star size={64} color="var(--red)" />
            <span>108</span>
          </div>
        </div>
        ...
        <div className="splash-subtitle" style={{
          ...
          fontSize: 18, color: '#fff', marginTop: 8, fontWeight: 700,
          letterSpacing: '0.3em',
        }}>
          コンビニ・イチマルハチ
        </div>
      ...
      {/* Cover-style poster of group */}
      <div className="splash-poster-frame" style={{
        marginTop: 30, width: 240, padding: 8,
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: 'var(--red)', color: '#fff', padding: '3px 9px',
            fontSize: 10, fontFamily: 'Bebas Neue', letterSpacing: '0.2em',
            border: '1.5px solid #fff',
          }}>NEW SINGLE</div>
        </div>
        <div style={{
          padding: '6px 4px 0', fontFamily: 'VT323, monospace', fontSize: 11,
      ...
      <div className="splash-tag" style={{
        ...
        <div style={{ fontSize: 13, opacity: 0.7, letterSpacing: '0.1em' }}>
      ...
      <button className="press splash-cta" onClick={onEnter} style={{
        ...
        color: 'var(--ink)',
        fontFamily: 'Bebas Neue',
        fontSize: 24, letterSpacing: '0.15em',
        padding: '18px 50px',
      ...
      <div className="splash-cta-line" style={{ marginTop: 10, color: 'var(--yellow)', fontFamily: 'VT323, monospace', fontSize: 14 }}>
...
// =============== AISLE / BROWSE ===============
const BURST_CLASSES = ['burst-yellow', 'burst-blue', 'burst-red', 'burst-orange'];
const BG_CLASSES = ['bg-pink', 'bg-mint', 'bg-cream', 'bg-sky', 'bg-peach', 'bg-lilac'];
...
function ProductCard({ p, inCart, onAdd, onRemove }) {
  ...
  const burstCls = BURST_CLASSES[hash % BURST_CLASSES.length];
  const bgCls = BG_CLASSES[(hash >> 2) % BG_CLASSES.length];
          ...
          {inCart ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <button onClick={onRemove} style={{
                ...
                fontFamily: 'Bebas Neue', fontSize: 14, lineHeight: 1,
                border: '1.5px solid var(--ink)',
              }}>−</button>
              <span style={{
                fontFamily: 'Bebas Neue', fontSize: 14, minWidth: 14, textAlign: 'center',
                background: '#fff', border: '1.5px solid var(--ink)', padding: '0 6px',
              }}>{inCart}</span>
              <button onClick={onAdd} style={{
                width: 22, height: 22, background: 'var(--red)', color: '#fff',
                fontFamily: 'Bebas Neue', fontSize: 14, lineHeight: 1,
                border: '1.5px solid var(--ink)',
              }}>+</button>
            </div>
          ) : (
            <button className="press" onClick={onAdd} style={{
    ...
    <div className="screen" style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontFamily: 'Bebas Neue', fontSize: 24, position: 'relative',
        }}>
          🛒
          {itemCount > 0 && (
            <div style={{
          ...
          <div key={item.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button onClick={() => removeItem(item.id)} style={{
                width: 26, height: 26, background: 'var(--ink)', color: '#fff', fontFamily: 'Bebas Neue', fontSize: 16,
              }}>−</button>
              <span style={{ fontFamily: 'Bebas Neue', fontSize: 16, minWidth: 16, textAlign: 'center' }}>{item.qty}</span>
              <button onClick={() => addItem(item.id)} style={{
                width: 26, height: 26, background: 'var(--pink)', color: '#fff', fontFamily: 'Bebas Neue', fontSize: 16,
              }}>+</button>
            </div>
          </div>
        ))}
...
function ReceiptScreen({ cart, onShare, onRestart }) {
  ...
  return (
    <div className="screen scanlines" style={{
      background: 'var(--ink)',
      paddingTop: 16, paddingBottom: 140,
      position: 'relative',
    }}>
      <PhoneChrome />
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            marginTop: 6, textShadow: '2px 2px 0 var(--ink)',
          }}>{match.name}</div>
          <div style={{ fontFamily: '"Zen Kaku Gothic New"', fontSize: 18, fontWeight: 900, marginTop: 2 }}>
            {match.jp}
          </div>
          <div style={{ fontSize: 12, marginTop: 10, opacity: 0.7, letterSpacing: '0.05em' }}>
            of ONE OR EIGHT
          </div>
          <div style={{
...
Object.assign(window, { SplashScreen, AisleScreen, CartScreen, ReceiptScreen });
