...
function ReceiptDesktop({ cart, onShare, onRestart }) {
  ...
  // Pick the winner among the highest-scoring members. When two or more members
  // tie, choose randomly between them so no one member gets favored by tag order.
  const matchId = React.useMemo(() => {
    const entries = Object.entries(tagCounts);
    if (entries.length === 0) return null;
    const topScore = Math.max(...entries.map(e => e[1]));
    const winners = entries.filter(e => e[1] === topScore).map(e => e[0]);
    return winners[Math.floor(Math.random() * winners.length)];
  }, [Object.keys(tagCounts).join(','), Object.values(tagCounts).join(',')]);
