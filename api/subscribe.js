...
//   MAILCHIMP_LIST_ID   — the audience/list ID (e.g. 9318507adb)
//   MAILCHIMP_SERVER_PREFIX — optional; auto-derived from API key if not set (e.g. us14)
//
...
module.exports = async (req, res) => {
  ...
  // ── Check env vars ──
  // Accept either MAILCHIMP_LIST_ID or MAILCHIMP_AUDIENCE_ID (same thing, different names)
  const API_KEY = process.env.MAILCHIMP_API_KEY;
  const AUDIENCE_ID = process.env.MAILCHIMP_LIST_ID || process.env.MAILCHIMP_AUDIENCE_ID;
  if (!API_KEY || !AUDIENCE_ID) {
    console.error('[subscribe] Missing env vars. Have API_KEY:', !!API_KEY, 'AUDIENCE_ID:', !!AUDIENCE_ID);
    return res.status(500).json({ ok: false, error: 'Server not configured. Please contact support.' });
  }

  // Datacenter: use MAILCHIMP_SERVER_PREFIX if set, else derive from API key suffix
